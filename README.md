...
The helper reads both the run and PR from `artifact-source-repository` (the caller repository by default). Before downloading or changing a release, it requires a `pull_request` run with the same head repository, branch, and SHA as the PR, and requires `commit-sha` to match that SHA. It also checks the run's PR list, or performs the branch lookup above when the list is empty. A stale run, mismatched PR, or ambiguous branch fails without uploading or pruning assets. The token must be able to read Actions runs and pull requests in the source repository. `release-repository` only selects where the release is stored.
